...
def test_se3_to_pose():
    ...
    se3_recreated = SE3.from_ros(pose)
...
def test_se3_to_pose_stamped():
    ...
    se3_recreated = SE3.from_ros(pose.pose)
    assert np.allclose(se3.t, se3_recreated.t)
    assert np.allclose(se3.R, se3_recreated.R)
...
def test_se3_to_transform():
    ...
    se3_recreated = SE3.from_ros(transform)
    assert np.allclose(se3.t, se3_recreated.t)
    assert np.allclose(se3.R, se3_recreated.R)
...
def test_se3_to_transform_stamped():
    ...
    se3_recreated = SE3.from_ros(transform.transform)
    assert np.allclose(se3.t, se3_recreated.t)
    assert np.allclose(se3.R, se3_recreated.R)

def test_so3_to_transform():
    so3 = SO3() # identity
    transform = so3.to_ros(as_tf=True)
...
def test_so3_to_transform_stamped():
    so3 = SO3() # identity
    header = Header()
    transform = so3.to_ros(header, as_tf=True)
